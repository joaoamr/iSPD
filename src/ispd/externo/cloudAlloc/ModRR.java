...
public class ModRR extends Alocacao {
    ...
    public void iniciar() {
        System.out.println("---------------------------------------");
        System.out.println("Alocador MOD - RR iniciado");
                ...
                if (num_escravos > 0) {//caso existam máquinas livres
                    ...
                    if (auxMaq instanceof CS_VMM) {
                        ...
                        if ((memoriaNecessaria <= memoriaMaq && discoNecessario <= discoMaq && procVM <= maqProc)) {
                            ...
                            System.out.println(auxVM.getId() + " enviada para " + auxMaq.getId());
                            //////////////////////////
                            CS_VMM vmm = auxVM.getVmmResponsavel();

            //trecho de teste
                            //System.out.println("atendimento da vm:" + vm.getId() + "na maquina:" + vm.getMaquinaHospedeira().getId());
                            //fim teste
                            int index = maq.getMestres().indexOf(vmm);
                            if (index == -1) {
                                List<CentroServico> caminhoVMM = auxMaq.getMenorCaminhoCloud(maq, vmm);
                                auxVM.setCaminhoVMM(caminhoVMM);
                            } else {
                                auxVM.setCaminhoVMM(maq.getCaminhoMestre().get(index));
                            }
                            VMM.enviarVM(auxVM);
                            System.out.println("---------------------------------------");
