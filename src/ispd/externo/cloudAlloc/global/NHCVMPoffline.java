 ...
 */
public class NHCVMPoffline extends AlocadorGlobal {
    ...
    private boolean bestFit(CS_VirtualMac vm, ArrayList<Integer> pms){
        ...
        for(int i = 1; i < pms.size(); i++){
            mem = this.pms[pms.get(i)].getMemoriaDisponivel() - vm.getMemoriaDisponivel();
            disk =  this.pms[pms.get(i)].getDiscoDisponivel() - vm.getDiscoDisponivel();
            if(mem > 0 && disk > 0){
