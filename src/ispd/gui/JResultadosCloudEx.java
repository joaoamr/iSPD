 ...
 */
public class JResultadosCloudEx extends javax.swing.JDialog {
    ...
    public JResultadosCloudEx(Metricas metricas, RedeDeFilasCloud rdf, List tarefas) {
        this.rdf = rdf;
     ...
     */
    public JResultadosCloudEx(Metricas metricas) {
        html.setMetricasGlobais(metricas.getMetricasGlobais());
        tabelaRecurso = setTabelaRecurso(metricas);
        getResultadosTarefas(metricas);
        html.setMetricasTarefas(metricas);
     ...
     */
    public JResultadosCloudEx(java.awt.Frame parent, Metricas metricas, RedeDeFilasCloud rdf, List<Tarefa> tarefas) {
        super(parent, true);
        this.rdf = rdf;
    ...
    private void initComponents() {
        ...
        Object[] colunas = {"Label", "Owner", "Host", "Processing performed", "Communication performed", "SLA Efficience"};
    ...
    private void jButtonSalvarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonSalvarActionPerformed
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                ...
                Logger.getLogger(JResultadosCloudEx.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    ...
    private String getResultadosGlobais(MetricasGlobais globais) {
        ...
        texto +=String.format("\tTotal of actives hosts = %d \n", globais.getTotalHostsAtivos());
        
        return texto;
    }

    private String getResultadosTarefas(Metricas metrica) {
        int totalTarefas = metrica.getNumTarefas();
        String texto = "\n\n\t\tTASKS\n ";
        ...
        texto += String.format("    Tasks sent: %d .\n", totalTarefas);
    ...
    private String getResultadosComunicacao(Metricas metrica){
        ...
        HashMap<Integer, Double> costperlevel = new HashMap<Integer, Double>();
        
        levels.put(0, 0);levels.put(1, 0);levels.put(2, 0);
        
        costperlevel.put(0, 0.0);costperlevel.put(1, 0.0);costperlevel.put(2, 0.0);
                
        String texto = "\n\n\t\tCOMMUNICATION OVERVIEW\n ";
        int n = rdf.getLinks().size();
        int usednodes = 0;
        ...
        for (int i = 0; i < n; i++){            
            if(rdf.getLinks().get(i).getMetrica().getMbitsTransmitidos() > 0){
                ...
                usednodes++;
                int c = levels.get(rdf.getLinks().get(i).getNivel());
                Double d = costperlevel.get(rdf.getLinks().get(i).getNivel()) + rdf.getLinks().get(i).getMetrica().getMbitsTransmitidos();
                c++;
                levels.put(rdf.getLinks().get(i).getNivel(), c);
                costperlevel.put(rdf.getLinks().get(i).getNivel(), d);
            }
        }
        texto += String.format("    Used nodes: %d.\n", usednodes);
        texto += String.format("    Edge: %d (%g).\n", levels.get(0), costperlevel.get(0)) ;
        texto += String.format("    Aggregation: %d (%g).\n", levels.get(1), costperlevel.get(1));
        texto += String.format("    Core: %d (%g).\n", levels.get(2), costperlevel.get(2));
    ...
    private Object[][] setTabelaRecurso(Metricas metricas) {
        ...
        if (metricas.getMetricasProcessamento() != null) {
            for (Map.Entry<String, MetricasProcessamento> entry : metricas.getMetricasProcessamento().entrySet()) {
                ...
                comu = 0.0;
                host = maq.getHostid();
                if(maq.getFatorSLA() == -1)
                    sla = "---";
                else
                    sla = Double.toString(maq.getFatorSLA());
                
                tabela.add(Arrays.asList(nome, prop, host, proc, comu, sla).toArray());
            }
        }
