 ...
 */
public class CS_VirtualMac extends CS_Processamento implements Mensagens {
    ...
    public static final int NETWORKAWARE = 5;
    public static final int BESTFIT = 7;
    ...
    private int type = BESTFIT;
    private int globalId;
    private double poderNecessario;
    private double fatorSLA;
    ...
    public CS_VirtualMac(String id, String proprietario, int numeroProcessadores, double memoria, double disco, String OS) {
        ...
        this.tempoChegada = new HashMap<Integer, Double>();
    }
    
    public CS_VirtualMac(String id, String proprietario, int numeroProcessadores, double poderNecessarioPorNucleo, double memoria, double disco, String OS) {
        super(id, proprietario, 0, numeroProcessadores, 0, 0);
        this.processadoresDisponiveis = numeroProcessadores;
        this.memoriaDisponivel = memoria;
        this.discoDisponivel = disco;
        ...
        this.caminhoMestre = null;
        this.tempoChegada = new HashMap<Integer, Double>();
        this.poderNecessario = poderNecessarioPorNucleo * numeroProcessadores;
    }
    ...
    public void chegadaDeCliente(Simulacao simulacao, Tarefa cliente) {
        ...
        if (cliente.getEstado() == Tarefa.CONCLUIDO) {
            this.vmMestre.getEscalonador().addTarefaConcluida(cliente);
            this.vmMestre.setTempoFinal(simulacao.getTime(this));
            return;
        }
        if (cliente.getEstado() != Tarefa.CANCELADO) { //se a tarefa estiver parada ou executando
            ...
            cliente.setNucleosAlocados(processadoresDisponiveis);
    ...
    public void atendimento(Simulacao simulacao, Tarefa cliente) {
        ...
        EventoFuturo evtFut = new EventoFuturo(
            ...
            maquinaHospedeira.getProcessadores(), cliente);
    ...
    public void saidaDeCliente(Simulacao simulacao, Tarefa cliente) {
        ...
        fatorSLA = (getMetrica().getMFlopsProcessados()/getMetrica().getSegundosDeProcessamento())/poderNecessario;
        getMetrica().setFatorSLA(fatorSLA);
        ...
        if(Origem.equals(vmMestre)){
            ...
            if(Origem.equals(this.vmmResponsavel)){
                ...
                caminho = (ArrayList<CentroServico>)CS_VMM.getMenorCaminhoCloud(maquinaHospedeira, ((CS_VirtualMac)Origem).getMaquinaHospedeira());
        ...
        while(cliente.subtarefas()){
            Tarefa sub = cliente.removerSubTarefa(0);
            sub.setUltimoCS(this);
   
            List<CentroServico> caminhosub = CS_VMM.getMenorCaminhoCloud(maquinaHospedeira, ((CS_VirtualMac)sub.getLocalProcessamento()).getMaquinaHospedeira());
            
            if(caminhosub == null || caminhosub.isEmpty()){
                caminhosub = new ArrayList<CentroServico>();
                caminhosub.add(maquinaHospedeira.getVirtualBridge());
                caminhosub.add(maquinaHospedeira);
            }
            
            sub.setCaminho(caminhosub);
            
            EventoFuturo evtSub = new EventoFuturo(
            simulacao.getTime(this),
            EventoFuturo.CHEGADA,
            sub.getCaminho().remove(0),
            sub);
            //Event adicionado a lista de evntos futuros
            simulacao.addEventoFuturo(evtSub);
        }
    ...
    public void setPoderNecessario(double podercontratado) {
        this.poderNecessario = podercontratado;
    }
    ...
    private void atualizarFatorSLA(double tempo){
        maquinaHospedeira.getProcessadores().recalcularMflops(tempo);
       
        
        for(Tarefa cliente: tarefaEmExecucao){
            double tempoProc = tempo - tempoChegada.get(cliente.getIdentificador());
            this.getMetrica().incSegundosDeProcessamento(tempoProc);
            tempoChegada.put(cliente.getIdentificador(), tempo);
            this.getMetrica().incMflopsProcessados(cliente.getMflopsProcessado());
            
            cliente.setTamProcessamento(cliente.getMflopsProcessado());
            cliente.setMflopsProcessado(0);
    ...
    public double getPoderNecessario() {
